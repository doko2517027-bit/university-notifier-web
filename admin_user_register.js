...
updateAccentColor(department?.value || "", major?.value || "");
...
function setupEvents() {
  ...
  [departmentGrade, majorGrade].filter(Boolean).forEach((select) => {
...
async function registerStudent() {
  ...
  const numberValue = studentNumber?.value.trim() || "";
  ...
  const studentPageIdValue = studentPageId?.value.trim() || "";

  const studentPagePasswordValue = studentPagePassword?.value || "";

  const activeMailPasswordValue = activeMailPassword?.value.trim() || "";

  const manabaPasswordValue = manabaPassword?.value.trim() || "";

  const appPasswordValue = appPassword?.value || "";

  const appPasswordConfirmValue = appPasswordConfirm?.value || "";

  if (!selectedDepartment && !selectedMajor) {
    alert("学科または専攻を選択してください。");
  ...
  if (!studentPageIdValue) {
    alert("学生ページIDを入力してください。");

    return;
  }

  if (!studentPagePasswordValue) {
    alert("学生ページパスワードを入力してください。");
  ...
  if (appPasswordValue !== appPasswordConfirmValue) {
    alert("アプリ用パスワードが一致しません。");

    return;
  }

  const validationResult = validateStudentNumber(
    numberValue,
    selectedDepartment,
    selectedMajor,
  );
  ...
  if (missingSettings.length > 0) {
    const continueRegistration = confirm(
      "一部機能が利用できません。\n\n" +
  ...
  const userRef = doc(db, "users", numberValue);
  ...
  try {
    ...
    const [
      ...
      encryptStudentPagePassword(studentPagePasswordValue),

      activeMailPasswordValue
        ? encryptData(activeMailPasswordValue)
        : Promise.resolve(""),

      manabaPasswordValue
        ? encryptData(manabaPasswordValue)
        : Promise.resolve(""),

      hashPassword(appPasswordValue),
    ]);
    ...
    await setDoc(userRef, {
      ...
      studentPageId: studentPageIdValue,

      studentPagePasswordEncrypted,

      activeMailPasswordEncrypted,

      manabaId: numberValue,
      ...
      notificationSettings: {
        schedule: notifySchedule?.checked ?? true,

        assignment: notifyAssignment?.checked ?? true,

        reminder: notifyReminder?.checked ?? true,

        courseNews: notifyCourseNews?.checked ?? true,
      ...
      manabaSetupSkipped: manabaPasswordValue === "",
    ...
    registerButton.textContent = "学生を登録する";
...
function validateStudentNumber(numberValue, selectedDepartment, selectedMajor) {
  if (!/^\d{7}$/.test(numberValue)) {
    return {
      valid: false,
      message: "学生番号は7桁の数字で入力してください。",
    };
  }
  ...
  if (
    ...
    return {
      valid: false,
      message: "学生番号の所属コードが正しくありません。",
    };
  }

  if (
    departmentCode === "10" &&
    (studentSequence < 1 || studentSequence > 200)
  ) {
    return {
      valid: false,
      message: "看護学科の学生番号が正しくありません。",
    };
  }

  if (
    (departmentCode === "20" || departmentCode === "30") &&
    (studentSequence < 1 || studentSequence > 60)
  ) {
    return {
      valid: false,
      message: "リハビリテーション学科の学生番号が正しくありません。",
    };
  }

  if (departmentCode === "10" && selectedDepartment !== "看護学科") {
    return {
      valid: false,
      message: "この学生番号は看護学科の番号です。",
    };
  }

  if (departmentCode === "20" && selectedMajor !== "理学療法学専攻") {
    return {
      valid: false,
      message: "この学生番号は理学療法学専攻の番号です。",
    };
  }

  if (departmentCode === "30" && selectedMajor !== "作業療法学専攻") {
    return {
      valid: false,
      message: "この学生番号は作業療法学専攻の番号です。",
    };
  }
...
const STUDENT_PAGE_SECRET = "UniversityNotifier2026";

async function encryptStudentPagePassword(plainText) {
  const encoder = new TextEncoder();

  const key = await crypto.subtle.importKey(
    "raw",
    encoder.encode(STUDENT_PAGE_SECRET.padEnd(32, "0")),
    "AES-GCM",
    false,
    ["encrypt"],
  );
