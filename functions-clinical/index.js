...
exports.authenticateClinical = onCall(
  ...
  async (request) => {
    ...
    if (
      claims.clinical !== true ||
      !ROLES.has(String(claims.clinicalRole || ""))
    ) {
      throw new HttpsError(
        "permission-denied",
        "Clinical権限が設定されていません。",
      );
    }
    ...
    return {
      token,
      role: claims.clinicalRole,
      hospitalId: claims.clinicalHospitalId,
    };
  },
);
...
async function requireHospitalAdministrator(request, hospitalId) {
  ...
  const hospitalOwner =
    actor.clinical === true &&
    actor.clinicalRole === "administrator" &&
    actor.clinicalHospitalId === hospitalId;
  if (!hospitalOwner) {
    throw new HttpsError(
      "permission-denied",
      "この病院の管理者ではありません。",
...
exports.createClinicalHospital = onCall(
  { region: "asia-northeast1" },
  async (request) => {
    ...
    try {
      ...
      if (error?.code === "auth/user-not-found")
        throw new HttpsError(
          "not-found",
          "病院管理者のCareMateアカウントが見つかりません。",
        );
      throw error;
    }
    await auth.setCustomUserClaims(administrator.uid, {
      ...(administrator.customClaims || {}),
      clinical: true,
      clinicalRole: "administrator",
      clinicalHospitalId: hospitalId,
    });
    const now = FieldValue.serverTimestamp();
    await db.doc(`clinicalHospitals/${hospitalId}`).set({
      hospitalId,
      hospitalName,
      administratorId,
    ...
    await db
      .doc(`clinicalHospitals/${hospitalId}/staff/${administrator.uid}`)
      .set({
        uid: administrator.uid,
        staffId: administratorId,
        role: "administrator",
        active: true,
        updatedAt: now,
        updatedBy: administratorId,
      });
    await db.doc(`clinicalHospitals/${hospitalId}/audit/hospital_created`).set({
      action: "hospital_created",
      hospitalName,
      administratorId,
      occurredAt: now,
      actorStudentNumber: administratorId,
    });
    await configRef.set({
      hospitalId,
      hospitalName,
...
exports.configureClinicalStaff = onCall(
  { region: "asia-northeast1" },
  async (request) => {
    const staffId = String(
      request.data?.staffId || request.data?.uid || "",
    ).trim();
    ...
    if (!uid || !/^[A-Za-z0-9_-]{3,80}$/.test(hospitalId) || !ROLES.has(role)) {
      throw new HttpsError(
        "invalid-argument",
        "職員・施設・職種の指定が正しくありません。",
      );
    }
    if (role === "administrator") {
      throw new HttpsError(
        "failed-precondition",
        "病院管理者は病院作成時に設定した一人だけです。",
      );
    }
    const actorStudentNumber = await requireHospitalAdministrator(
      request,
      hospitalId,
    );
    let user;
    ...
    await db.doc(`clinicalHospitals/${hospitalId}/staff/${uid}`).set(
      {
        ...
        updatedBy: actorStudentNumber,
      },
      { merge: true },
    );
    await db.collection(`clinicalHospitals/${hospitalId}/audit`).add({
      action: active ? "staff_granted" : "staff_revoked",
      targetUid: uid,
      role,
...
exports.listClinicalStaff = onCall(
  { region: "asia-northeast1" },
  async (request) => {
    const hospitalId = String(request.data?.hospitalId || "").trim();
    ...
    const snapshot = await db
      .collection(`clinicalHospitals/${hospitalId}/staff`)
      .get();
    return {
      staff: snapshot.docs
        .map((item) => {
...
exports.resetClinicalFoundation = onCall(
  { region: "asia-northeast1" },
  async (request) => {
