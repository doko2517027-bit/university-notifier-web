...
const list = document.getElementById("personalTimetableList"),
  count = document.getElementById("timetableCourseCount");
const escapeHtml = (v) =>
  String(v ?? "").replace(
    /[&<>"']/g,
    (c) =>
...
const formatDate = (value, fallback) => {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(value)) return fallback || value;
...
let userPreferences = {};
async function preference(subject) {
  if (!Object.keys(userPreferences).length)
    userPreferences =
      (await getDoc(doc(db, "users", studentNumber))).data()
        ?.attendancePreferences || {};
  return userPreferences[encodeURIComponent(subject)]?.classGroup || "";
}
async function chooseGroup(subject, group) {
  if (!confirm(`${subject}の通知を「${group}」に設定しますか？`)) return;
  await updateDoc(doc(db, "users", studentNumber), {
    [`attendancePreferences.${encodeURIComponent(subject)}`]: {
      subject,
      classGroup: group,
...
async function load() {
  try {
    ...
    list.innerHTML = [...groups.entries()]
      .map(
        ([key, entries]) =>
          `<section class="personal-timetable-day"><div class="personal-timetable-date"><b>${escapeHtml(formatDate(key, entries[0].dayTitle))}</b><span>${entries.length}コマ</span></div>${entries
            .map((entry) => {
    ...
    list
      ...
      .forEach(
        (button) =>
          (button.onclick = () =>
            chooseGroup(button.dataset.subject, button.dataset.group).catch(
              (error) => alert(error.message),
            )),
      );
  } catch (error) {
    console.error("個人時間割取得エラー", error);
    list.innerHTML =
      '<div class="card setting-card">時間割を取得できませんでした。</div>';
  }
}
await initializePage([
  loadProfileImage(document.getElementById("topProfileImage")),
  load(),
]);
