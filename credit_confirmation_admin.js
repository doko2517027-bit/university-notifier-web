...
import { collection, doc, getDoc, getDocs, setDoc, serverTimestamp } from "https://www.gstatic.com/firebasejs/12.2.1/firebase-firestore.js";
...
const currentAcademicYear = () => new Date().getMonth() < 3 ? new Date().getFullYear() - 1 : new Date().getFullYear();
...
async function load() {
  try {
    const [systemSnap, usersSnap] = await Promise.all([getDoc(doc(db, "system", "app")), getDocs(collection(db, "users"))]);
    ...
    help.textContent = active ? `${academicYear}年度 ${semester}の単位取得確認を受付中です。` : "開始すると、対象学生は次回ホームを開いた時に回答します。";
    if (!active) { list.innerHTML = "<p>単位取得確認はまだ開始していません。</p>"; return; }
    const users = usersSnap.docs.map(item => ({ id:item.id, ...item.data() })).filter(user => matchesAdminScope(user, scope)).sort((a,b) => String(a.id).localeCompare(String(b.id)));
    list.innerHTML = users.map(user => {
      const answer = user.creditConfirmationResponse;
      const answered = Number(answer?.academicYear) === academicYear && answer?.semester === semester;
...
async function saveConfig(enabled) {
  ...
  if (enabled && (!Number.isInteger(academicYear) || !["前期", "後期"].includes(semester))) { showToast("年度と学期を選択してください"); return; }
  if (!confirm(enabled ? `${academicYear}年度 ${semester}の単位取得確認を開始しますか？` : "単位取得確認を停止しますか？")) return;
  try {
    await setDoc(doc(db, "system", "app"), { creditConfirmation: { enabled, academicYear, semester, updatedAt:new Date().toISOString(), updatedBy:studentNumber || "" }, updatedAt:serverTimestamp() }, { merge:true });
    showToast(enabled ? "単位取得確認を開始しました" : "単位取得確認を停止しました");
    await load();
  } catch (error) { console.error("単位取得確認の保存エラー:", error); showToast("保存に失敗しました"); }
}
