...
// 2026-07-08 1限の個人時間割を使う、一回限りの本番Push疎通テスト。
// 固定された履修済み授業以外には送らず、出席記録も作成しない。
exports.sendJuly8AttendanceTest = onRequest({
    ...
    const userId = "2510044";
    const testDate = "2026-07-08";
    const periodNumber = 1;
    const scheduleId = "ns_yamate";
    const dispatchRef = db.collection("attendanceNotificationDispatches")
        .doc("attendance_test_2510044_2026-07-08_P1_v1");

    const previous = await dispatchRef.get();
    if (previous.exists) {
        response.json({ ok: true, duplicateSuppressed: true, ...previous.data() });
        return;
    }

    const [userSnap, enrolledSnap, scheduleSnap] = await Promise.all([
        db.collection("users").doc(userId).get(),
        db.collection("users").doc(userId).collection("enrolledSubjects").get(),
        db.collection("schedule").doc(scheduleId).get()
    ]);
    if (!userSnap.exists || !scheduleSnap.exists) {
        response.status(404).json({ ok: false, error: "user or schedule missing" });
        return;
    }
    const user = userSnap.data() || {};
    const enrolled = new Set();
    enrolledSnap.docs.forEach(document => {
        const item = document.data();
        if (item.status === "removed") return;
        [document.id, item.name, item.subjectKey, item.subjectId].forEach(value => {
            const normalized = normalizeCourseName(value);
            if (normalized) enrolled.add(normalized);
        });
    });
    const day = (scheduleSnap.data().allDays || []).find(item => item.date === testDate);
    const grade = String(user.grade || "").replace("年", "").trim();
    const lecture = (day?.schedules || []).find(item =>
        Number.parseInt(item.period, 10) === periodNumber &&
        enrolled.has(normalizeCourseName(item.subject)) &&
        (!grade || String(item.grade || "").replace("年", "").trim() === grade)
    );
    if (!lecture) {
        response.status(404).json({ ok: false, error: "enrolled lecture missing" });
        return;
    }

    const recordId = slotId(scheduleId, testDate, periodNumber, lecture.subject);
    const startTime = lecture.startTime || PERIOD_TIMES[periodNumber].startTime;
    const endTime = lecture.endTime || PERIOD_TIMES[periodNumber].endTime;
    const classGroup = lecture.classGroup || "";
    const query = new URLSearchParams({
        action: "arrival", recordId, scheduleId, date: testDate,
        period: String(periodNumber), subject: lecture.subject,
        classGroup, startTime, endTime, attendanceTest: "1"
    }).toString();
    ...
    await db.runTransaction(async transaction => {
        ...
        transaction.create(dispatchRef, {
            test: true, userId, scheduleId, date: testDate, period: periodNumber,
            subject: lecture.subject, classGroup, simulatedAt: "08:49",
            recordCreated: false, createdAt: new Date()
        });
        claimed = true;
    });
    if (!claimed) {
        response.json({ ok: true, duplicateSuppressed: true });
        return;
    }
    ...
    const results = await sendToUserDevices(userId, {
        title: `🧪 出席確認テスト${classGroup ? `（${classGroup}）` : ""}`,
        body: `${lecture.subject}：7月8日1限・08:49テスト（出席記録は作成されません）`,
        url: `${SITE_URL}/attendance_check.html?${query}`,
        tag: `attendance-test-${recordId}`
    });
    await dispatchRef.update({ results, sentAt: new Date() });
    response.json({ ok: true, duplicateSuppressed: false, userId, subject: lecture.subject,
        classGroup, date: testDate, period: periodNumber, startTime, results });
});
