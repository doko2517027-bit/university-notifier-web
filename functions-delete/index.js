...
exports.deleteCareMateUser = onCall(
    ...
    async request => {
        ...
        if (request.auth?.token?.admin !== true || actor !== "2510044") {
            throw new HttpsError("permission-denied", "この操作を実行できる管理者ではありません。");
        }
        if (!/^\d{7}$/.test(target)) {
            throw new HttpsError("invalid-argument", "学籍番号が正しくありません。");
        }
        if (target === actor) {
            throw new HttpsError("failed-precondition", "ログイン中の管理者自身は削除できません。");
        }

        const removeDocument = reference => db.recursiveDelete(reference);
        const removeMatches = async collectionName => {
            const records = await db.collection(collectionName).where("studentNumber", "==", target).get();
            await Promise.all(records.docs.map(record => removeDocument(record.ref)));
            return records.size;
        };
        const directCollections = [
            "users", "publicUsers", "courseLinks", "courseNews", "userPresence",
            "attendance", "attendancePreferences", "attendanceRecords", "examProgress",
            "subjectPoints", "totalRanking"
        ];
