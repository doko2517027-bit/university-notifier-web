...
const adminFeatureRequestCard = document.getElementById(
  "adminFeatureRequestCard",
);
const adminFeatureRequestBadge = document.getElementById(
  "adminFeatureRequestBadge",
);
if (studentNumber === "2510044" && adminContactCard) {
  adminContactCard.hidden = false;
  onSnapshot(collection(db, "contacts"), (snapshot) => {
    const count = snapshot.docs.filter(
      (item) => !["done", "resolved"].includes(item.data().status || "new"),
    ).length;
...
if (studentNumber === "2510044" && adminFeatureRequestCard) {
  ...
  onSnapshot(collection(db, "featureRequests"), (snapshot) => {
    const count = snapshot.docs.filter((item) =>
      ["submitted", "reviewing", "developing"].includes(
...
const lastAssignmentCheckText = document.getElementById(
...
function renderAdminScope() {
  ...
  if (adminScopeCurrent) {
    adminScopeCurrent.textContent = `現在の管理対象：${scopeLabel(scope)}`;
  }
...
function getSystemAppSnapshot() {
  if (!systemAppPromise) {
    systemAppPromise = getDoc(doc(db, "system", "app"));
  }
...
if (adminScopeDepartment)
  adminScopeDepartment.value = storedAdminScope.department;
...
async function loadDashboard() {
  setText(versionText, `Version ${VERSION}`);
  ...
  try {
    ...
    setText(firestoreStatus, "🟢 正常");
  } catch (error) {
    console.error("ダッシュボード取得エラー:", error);

    setText(userCount, "取得失敗");

    setText(firestoreStatus, "🔴 エラー");
  }
}
...
async function loadSystemStatus() {
  try {
    const snapshot = await getSystemAppSnapshot();

    if (!snapshot.exists()) {
      setText(renderStatus, "⚫ 未確認");

      setText(lastCronText, "----");

      setText(lastScheduleCheckText, "----");

      setText(lastAssignmentCheckText, "----");
    ...
    setText(renderStatus, renderStatusText[data.renderStatus] || "⚫ 未確認");

    setText(lastCronText, formatAdminDate(data.lastCronAt));

    setText(lastScheduleCheckText, formatAdminDate(data.lastScheduleCheckAt));
...
async function loadMaintenance() {
  try {
    ...
    console.error("メンテナンス設定取得エラー:", error);
  }
}

async function saveMaintenanceSettings() {
  if (!maintenanceToggle || !maintenanceMessage || !saveMaintenance) {
    return;
  }

  saveMaintenance.disabled = true;

  saveMaintenance.textContent = "保存中...";
  ...
  try {
    ...
    saveMaintenance.textContent = "保存する";
  }
}
...
async function loadNotificationSettings() {
  ...
  try {
    const snapshot = await getDoc(doc(db, "users", studentNumber));
    ...
    setChecked(notifySchedule, settings.schedule ?? true);

    setChecked(notifyAssignment, settings.assignment ?? true);

    setChecked(notifyReminder, settings.reminder ?? true);

    setChecked(notifyCourseNews, settings.courseNews ?? true);
  ...
  try {
    await updateDoc(doc(db, "users", studentNumber), {
      notificationSettings: {
        schedule: getChecked(notifySchedule, true),

        assignment: getChecked(notifyAssignment, true),

        reminder: getChecked(notifyReminder, true),

        courseNews: getChecked(notifyCourseNews, true),
...
function setupEvents() {
  [adminScopeDepartment, adminScopeMajor, adminScopeGrade]
    .filter(Boolean)
    .forEach((input) => {
      input.addEventListener("change", renderAdminScope);
    });

  document.querySelectorAll(".admin-menu-card[onclick]").forEach((card) => {
    const match = card
  ...
  if (sendTestPush) {
    sendTestPush.onclick = sendTestNotification;
  }
...
async function enablePushNotification() {
  try {
    await registerDevicePushSubscription(db, studentNumber, "admin", "sw.js");

    showToast("通知を再登録しました");
  } catch (error) {
    console.error("Push通知登録エラー:", error);

    alert(error.message || "通知の登録に失敗しました。");
  }
}
...
function getChecked(element, fallback) {
  if (!element) {
    return fallback;
  }
