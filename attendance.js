...
async function loadAttendance(){
    const [recordSnap,subjectSnap]=await Promise.all([
        getDocs(query(collection(db,"users",studentNumber,"attendanceRecords"),orderBy("date","desc"))),
    ...
    for(const recordDoc of recordSnap.docs){const record={id:recordDoc.id,...recordDoc.data()};const key=record.subjectKey||record.subject||"科目未設定";if(!groups.has(key))groups.set(key,[]);groups.get(key).push(record)}
