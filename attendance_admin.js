...
const currentSemesterSelect = document.getElementById(
  "attendanceCurrentSemester",
);

const currentSemesterSaveButton = document.getElementById(
  "saveAttendanceCurrentSemester",
);

const currentSemesterResult = document.getElementById(
...
const timeAt = (minutes) =>
  `${two(Math.floor(minutes / 60) % 24)}:${two(minutes % 60)}`;
...
if (!(await isAdmin())) {
  ...
  attendanceScopeLabel.textContent = `現在の管理対象：${scopeLabel(
    adminScope,
  )}`;

  await loadAttendanceCurrentSemester();

  if (currentSemesterSaveButton) {
    currentSemesterSaveButton.onclick = saveAttendanceCurrentSemester;
  }
  ...
  [startInput, endInput].forEach((node) =>
    node.addEventListener("input", updateTestExplanation),
  );
  ...
  document.getElementById("loadStudentAttendance").onclick =
...
async function loadAttendanceCurrentSemester() {
  try {
    ...
    const semester = snap.exists() ? snap.data().attendanceCurrentSemester : "";

    if (semester === "前期" || semester === "後期") {
      if (currentSemesterSelect) {
        currentSemesterSelect.value = semester;
      }

      if (currentSemesterResult) {
        currentSemesterResult.textContent = `現在学期：${semester}`;
      }
    ...
    if (currentSemesterResult) {
      currentSemesterResult.textContent = "現在学期がまだ設定されていません。";
    }
  } catch (error) {
    console.error("現在学期取得エラー:", error);

    if (currentSemesterResult) {
      currentSemesterResult.textContent = "現在学期を読み込めませんでした。";
    }
  }
}

async function saveAttendanceCurrentSemester() {
  if (!currentSemesterSelect || !currentSemesterSaveButton) {
    return;
  }
  ...
  if (semester !== "前期" && semester !== "後期") {
    showToast("現在学期を選択してください");

    return;
  }

  currentSemesterSaveButton.disabled = true;

  try {
    await setDoc(
      doc(db, "system", "app"),
      {
        attendanceCurrentSemester: semester,

        attendanceCurrentSemesterUpdatedAt: serverTimestamp(),

        attendanceCurrentSemesterUpdatedBy: studentNumber || "",
      },
      {
        merge: true,
      },
    ...
    showToast("現在学期を保存しました");
  } catch (error) {
    console.error("現在学期保存エラー:", error);

    showToast("現在学期を保存できませんでした");
  } finally {
    currentSemesterSaveButton.disabled = false;
  }
}
...
function updateTestExplanation() {
  if (!startInput.value || !endInput.value) {
    testExplanation.textContent = "開始・終了時刻を入力してください。";
  ...
  const notify = timeAt(hour * 60 + minute - 10);
...
function updateClassGroupOptions() {
  ...
  if (!count) {
    classGroupSelect.innerHTML = '<option value="">クラスなし</option>';

    classGroupSelect.disabled = true;

    return;
  }

  classGroupSelect.innerHTML = Array.from(
    {
      length: count,
    },
    (_, index) => {
      ...
      return `<option value="${group}">` + `${group}クラス` + `</option>`;
    },
  ).join("");
...
async function loadEnrolledSubjects() {
  ...
  if (!user.exists()) {
    subjectSelect.innerHTML = "<option>登録されていない学籍番号です</option>";
  ...
  if (!matchesAdminScope(user.data(), adminScope)) {
    subjectSelect.innerHTML = "<option>現在の管理対象外の学生です</option>";
  ...
  const enrolled = await getDocs(
    collection(db, "users", number, "enrolledSubjects"),
  );
  ...
  const subjects = [
    ...new Set(
      enrolled.docs
        ...
        .map(
          (item) =>
            item.name || item.subject || item.subjectKey || item.subjectId,
        )
  ...
  if (!subjects.length) {
    subjectSelect.innerHTML = "<option>履修科目がありません</option>";
  ...
  subjectSelect.innerHTML = subjects
    .map(
      (subject) =>
        `<option value="${escapeHtml(subject)}">${escapeHtml(
          subject,
        )}</option>`,
    )
    .join("");
...
async function queueTestNotification() {
  ...
  const date =
    `${today.getFullYear()}-` +
    `${two(today.getMonth() + 1)}-` +
    `${two(today.getDate())}`;

  const expiresAt = new Date(today.getTime() + 20 * 60 * 1000).toISOString();
  ...
  try {
        ...
        lectures: [
          {
            ...
            classGroup: classGroupSelect.value,
    ...
    testResult.textContent =
      `${number} の「${subject}」を予約しました。` + `開始10分前に通知します。`;

    showToast("テスト通知を予約しました");
  } catch (error) {
    console.error(error);

    showToast("予約に失敗しました。Firebaseルールを確認してください");

    testResult.textContent = "予約に失敗しました。";
  } finally {
    sendButton.disabled = false;
  }
}
...
function reviewCard(item) {
  ...
  return `
    <article
      ...
      <p>
        学生：
        ${escapeHtml(data.studentNumber || item.ref.parent.parent?.id || "-")}
        <br>

        日時：
        ${escapeHtml(data.date || "-")}
        ${escapeHtml(data.period || "-")}限
        <br>

        打刻：
        ${escapeHtml(data.endLabel || "-")}
        （${formatDate(data.endClientAt)}）
      </p>
...
async function watchPendingReviews() {
  try {
    ...
    const scopedUsers = users.docs.filter((user) =>
      matchesAdminScope(user.data(), adminScope),
    );

    const snapshots = await Promise.all(
      scopedUsers.map((user) =>
        getDocs(collection(user.ref, "attendanceRecords")),
      ),
    );
    ...
    const pending = attendanceRecords.filter(
      (item) =>
        item.data().earlyEndReviewRequired === true &&
        item.data().earlyEndReviewStatus === "pending",
    );

    pending.sort(
      (left, right) =>
        (right.data().updatedAt?.toMillis?.() || 0) -
        (left.data().updatedAt?.toMillis?.() || 0),
    );
    ...
    pendingList.innerHTML =
      pending.map(reviewCard).join("") || "<p>確認待ちの記録はありません。</p>";
  } catch (error) {
    console.error(error);

    pendingList.innerHTML = "<p>確認待ちの記録を読み込めません。</p>";
  }
}
...
async function handleReviewAction(event) {
    ...
    try {
      await updateDoc(
        ...
        ),
        {
          ...
          adminEditedBy: studentNumber || "",

          updatedAt: serverTimestamp(),
        },
      );

      showToast("出席記録を変更しました");
  ...
  if (bulkButton) {
    ...
    const matching = attendanceRecords.filter((item) => {
      const data = item.data();

      return (
        data.date === card.dataset.date &&
        String(data.period) === card.dataset.period &&
        data.subject === card.dataset.subject &&
        String(data.testId || "") === card.dataset.testId &&
        Boolean(data.startStampedAt || data.startKind) &&
        !data.absenceTapped
      );
    });

    if (!matching.length) {
      showToast("開始打刻済みの学生がいません");
    ...
    try {
      await Promise.all(
        matching.map((item) =>
          updateDoc(item.ref, {
            earlyEndReviewStatus: "resolved",

            earlyEndReviewResolvedAt: serverTimestamp(),

            earlyEndReviewResolvedBy: studentNumber || "",

            earlyEndReviewResolution: "present_bulk",
      ...
      showToast(`${matching.length}人を出席として確定しました`);
  ...
  try {
    await updateDoc(doc(db, "users", student, "attendanceRecords", recordId), {
      earlyEndReviewStatus: "resolved",
      ...
      earlyEndReviewResolution: status,
    ...
    showToast("確定に失敗しました。Firebaseルールを確認してください");
...
async function loadStudentAttendance() {
  ...
  try {
    ...
    if (!user.exists() || !matchesAdminScope(user.data(), adminScope)) {
      recordStudentList.innerHTML = "<p>現在の管理対象外の学生です。</p>";

      return;
    }

    const records = await getDocs(
      collection(db, "users", number, "attendanceRecords"),
    );

    const rows = records.docs.sort(
      (left, right) =>
        String(right.data().date || "").localeCompare(
          String(left.data().date || ""),
        ) || Number(right.data().period || 0) - Number(left.data().period || 0),
    );
    ...
    recordStudentList.innerHTML =
      rows
        .map((item) => {
          ...
          const current = data.status || "unrecorded";

          return `
              <article
                class="attendance-review-card"
                data-student-record="${escapeHtml(item.id)}">
    ...
    recordStudentList.innerHTML = "<p>出席記録を読み込めません。</p>";
  }
}
