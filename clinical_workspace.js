...
document.head.insertAdjacentHTML(
  "beforeend",
  `<style>.clinical-notice{display:none!important}</style>`,
);
...
if (claims.clinical !== true) {
  $("workspace").innerHTML =
    '<div class="clinical-empty">Clinicalへログインしてから開いてください。</div>';
  throw new Error("Clinical login required");
}

const currentRole = String(claims.clinicalRole || "");
const hospitalId = String(claims.clinicalHospitalId || "");
$("roleLabel").textContent =
  `${roleName[currentRole] || "Clinical職員"} ・ ${hospitalId}`;
if (currentRole === "administrator")
  document
    .querySelector("header.header")
    .insertAdjacentHTML(
      "beforeend",
      '<p><a class="btn btn-secondary" href="clinical_admin.html">病院管理</a></p>',
    );

const canManage = ["administrator", "clerk"].includes(currentRole);
const canEdit =
  editableRoles.includes(currentRole) || currentRole === "administrator";
if (canManage) $("addPatient").classList.remove("hidden");
if (canManage)
  document
    .querySelector(".clinical-sidebar")
    .insertAdjacentHTML(
      "beforeend",
      '<div class="clinical-toolbar"><button id="setICloudFile" class="btn btn-secondary">iCloudフォルダを設定</button><button id="loadICloudFile" class="btn btn-secondary">iCloudから読み込み</button><small id="cloudStatus">保存先は未設定です</small></div>',
    );
...
function patientAge(birth) {
  ...
  if (Number.isNaN(date.getTime())) return "年齢未設定";
...
const allowEditor = () =>
  currentRole === "administrator" || currentRole === editorRole;
...
function openCloudDB() {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open(cloudDB, 1);
...
async function storeFileHandle(handle) {
  ...
  tx.objectStore("handles").put(handle, "patientFile");
  return new Promise((resolve) => (tx.oncomplete = resolve));
}
async function readFileHandle() {
  const db = await openCloudDB();
  const tx = db.transaction("handles", "readonly");
  const request = tx.objectStore("handles").get("patientFile");
  return new Promise((resolve) => {
    request.onsuccess = () => resolve(request.result || null);
    request.onerror = () => resolve(null);
  });
}
const patientFileName = (patient) =>
  `caremate-clinical-${String(patient.key).replace(/[^A-Za-z0-9_-]/g, "")}.json`;
function normalizePatients(list) {
  return list.map((patient) => ({
    ...patient,
    key: patient.key || crypto.randomUUID(),
    vitals: patient.vitals || [],
    vitalSeries: patient.vitalSeries || [],
    calendarEvents: patient.calendarEvents || [],
    records: patient.records || [],
  }));
}
async function saveToICloud() {
  if (!clinicalDirectoryHandle) return;
  try {
    for (const patient of patients) {
      const fileHandle = await clinicalDirectoryHandle.getFileHandle(
        patientFileName(patient),
        { create: true },
      );
      const writable = await fileHandle.createWritable();
      await writable.write(JSON.stringify({ version: 1, patient }, null, 2));
      await writable.close();
    }
    cloudStatus(`${patients.length}人を患者別JSONで保存済み`);
  } catch (error) {
    cloudStatus("保存できませんでした");
  }
}
async function loadFromICloud(handle = clinicalDirectoryHandle) {
  if (!handle) return;
  try {
    const loaded = [];
    for await (const [name, fileHandle] of handle.entries()) {
      if (!name.startsWith("caremate-clinical-") || !name.endsWith(".json"))
        continue;
      const data = JSON.parse(await (await fileHandle.getFile()).text());
      if (data.patient) loaded.push(data.patient);
    }
    patients = normalizePatients(loaded);
    selectedId = patients[0]?.key || "";
    cloudStatus(`${patients.length}人をiCloudから読み込み済み`);
    render();
  } catch (error) {
    alert("患者別保存ファイルを読み込めませんでした。");
  }
}
async function configureICloudFile() {
  if (!window.showDirectoryPicker) {
    alert("Chromeで開いてください。");
    return;
  }
  try {
    clinicalDirectoryHandle = await window.showDirectoryPicker({
      mode: "readwrite",
    });
    await storeFileHandle(clinicalDirectoryHandle);
    await saveToICloud();
    cloudStatus("iCloudフォルダを設定済み");
  } catch (error) {}
}
async function chooseICloudFile() {
  if (!window.showDirectoryPicker) {
    alert("Chromeで開いてください。");
    return;
  }
...
async function restoreICloudFile() {
  clinicalDirectoryHandle = await readFileHandle();
  if (!clinicalDirectoryHandle) return;
  if (clinicalDirectoryHandle.kind !== "directory") {
    clinicalDirectoryHandle = null;
    cloudStatus("患者別JSON用にiCloudフォルダを設定してください");
    return;
  }
  const permission = await clinicalDirectoryHandle.queryPermission({
    mode: "readwrite",
  });
  if (permission === "granted") await loadFromICloud();
  else cloudStatus("iCloudフォルダを再選択してください");
}

function renderPatients() {
  const keyword = $("patientSearch").value.trim().toLowerCase();
  const rows = patients.filter((patient) =>
    `${patient.id} ${patient.label}`.toLowerCase().includes(keyword),
  );
  $("patientList").innerHTML = rows.length
    ? rows
        .map(
          (patient) =>
            `<button class="clinical-patient ${patient.key === selectedId ? "active" : ""}" data-patient="${patient.key}"><b>${esc(patient.label)}</b><small>患者番号：${esc(patient.id)} ・ ${esc(patient.department)} ・ ${esc(patient.status)}</small></button>`,
        )
        .join("")
    : "<small>該当する患者はいません。</small>";
}

function tabButtons() {
  return `<div class="clinical-tabs">${[
    ["overview", "患者サマリー"],
    ["calendar", "時系列カレンダー"],
    ["entry", "職種別入力"],
    ["records", "多職種カルテ"],
...
function emptyPatientPanel() {
  return `<div class="clinical-empty"><h2>患者が登録されていません</h2><p>左の「患者を追加」から登録できます。</p><button id="startPatientRegistration" class="btn btn-primary">患者を追加</button></div>`;
}

function rolePicker() {
  if (currentRole !== "administrator")
    return `<span class="clinical-role-chip">${esc(roleName[currentRole] || "職員")}</span>`;
...
function entryPanel() {
  if (!canEdit)
    return `<h3>職種別入力</h3><div class="clinical-empty">この職種は記録の閲覧のみです。</div>`;
  let form =
    editorRole === "nurse"
      ? nurseEditor()
      : editorRole === "doctor"
        ? doctorEditor()
        : editorRole === "pharmacist"
          ? pharmacyEditor()
          : ["pt", "ot", "st"].includes(editorRole)
            ? rehabEditor()
            : clerkEditor();
  const now = new Date();
...
function recordsPanel(patient) {
  if (editingRecordIndex !== null) {
    ...
    if (!record) {
      ...
      return recordsPanel(patient);
    }
    const fields =
      record.role === "nurse"
        ? nurseEditor(record)
        : `<label>記録内容<textarea id="recordEditBody">${esc(record.body)}</textarea></label><label>補足・詳細<textarea id="recordEditDetails">${esc(record.details || "")}</textarea></label>`;
...
function ordersPanel(patient) {
  const canEditOrders =
    currentRole === "administrator" || currentRole === "doctor";
...
function chartsPanel(patient) {
  ...
  const graph = (label, key, unit) => {
    ...
    if (!points.length)
      return `<section class="clinical-record-type"><h3>${label}</h3><p class="clinical-note">記録はありません。</p></section>`;
    const min = Math.floor(Math.min(...points.map((item) => item.value)));
    const max =
      Math.ceil(Math.max(...points.map((item) => item.value))) +
      (points.length === 1 ? 1 : 0);
...
function patientEvents(patient) {
  return [
    ...(patient.calendarEvents || []),
    ...patient.records
      .filter((record) => record.date)
      .map((record) => ({
        date: record.date,
        time: record.time,
        type: ["pt", "ot", "st"].includes(record.role) ? "rehab" : record.role,
        label: `${roleName[record.role] || "職員"}：${record.title}`,
        detail: record.body,
        recordId: record.id,
      })),
  ];
...
function calendarPanel(patient) {
  ...
  const days = Array.from({ length: 42 }, (_, offset) => {
    ...
    const inside = date.getMonth() === monthStart.getMonth();
    const items = events.filter((item) => item.date === key);
    return `<button class="clinical-calendar-day ${inside ? "" : "empty"} ${calendarDay === key ? "selected" : ""}" data-calendar-day="${key}" ${inside ? "" : "tabindex=-1"}><time>${date.getDate()}</time><span class="clinical-calendar-events">${items
      .slice(0, 3)
      .map(
        (item) =>
          `<span class="clinical-calendar-event ${esc(item.type)}">${esc(item.label)}</span>`,
      )
      .join(
        "",
      )}${items.length > 3 ? `<small>+${items.length - 3}件</small>` : ""}</span></button>`;
  }).join("");
  ...
  const selectedLabel = new Intl.DateTimeFormat("ja-JP", {
    ...
    day: "numeric",
    weekday: "short",
  }).format(new Date(`${calendarDay}T00:00:00`));
...
function saveRoleEntry() {
  const patient = currentPatient();
  if (event.target.id === "startPatientRegistration") {
    tab = "register";
    render();
    return;
  }
  ...
  if (editorRole === "nurse") {
    ...
    vitals = {
      temp: value("vitalTemp"),
      pulse: value("vitalPulse"),
      respiratory: value("vitalRespiratory"),
      spo2: value("vitalSpo2"),
      sys: value("vitalSys"),
      dia: value("vitalDia"),
      height: value("vitalHeight"),
      weight: value("vitalWeight"),
      pain: value("painScale"),
      consciousness: value("consciousness"),
      intake: value("intake"),
      output: value("output"),
      menstrualLast: value("menstrualLast"),
      menstrualEnd: value("menstrualEnd"),
      menstrualCycle: value("menstrualCycle"),
      menstrualFlow: value("menstrualFlow"),
      menstrualPain: value("menstrualPain"),
      menstrualSymptoms: value("menstrualSymptoms"),
    ...
    body = value("medicalRecord");
    details = `主病名・診断名：${value("diagnosis")}`;
    const newOrders = value("doctorOrders");
    if (newOrders) patient.orders = newOrders;
  } else if (editorRole === "pharmacist") {
    body = value("pharmacyAssessment");
    details = `服薬状況：${value("medicationHistory")}\n医師への提案：${value("pharmacySuggestion")}`;
  } else if (["pt", "ot", "st"].includes(editorRole)) {
    body = value("rehabIntervention");
    details = `評価：${value("rehabAssessment")}\n目標：${value("rehabGoal")}\n次回計画：${value("rehabPlan")}`;
  } else {
    body = value("clerkRecord");
    details = `連絡事項：${value("clerkNotice")}`;
  }
  ...
  patient.records.unshift({
    ...
    time: entryTime,
    date: entryDate,
    vitals: editorRole === "nurse" ? vitals : undefined,
  });
  saveToICloud();
  tab = "records";
  render();
...
document.addEventListener("change", (event) => {
  if (event.target.id === "editorRole") {
    editorRole = event.target.value;
    render();
  }
});
document.addEventListener("click", async (event) => {
  ...
  if (patientButton) {
    selectedId = patientButton.dataset.patient;
    tab = "overview";
    editingRecordIndex = null;
    render();
    return;
  ...
  if (calendarDayButton) {
    calendarDay = calendarDayButton.dataset.calendarDay;
    render();
    return;
  }
  if (calendarNavButton) {
    calendarMonth = new Date(
      calendarMonth.getFullYear(),
      calendarMonth.getMonth() + Number(calendarNavButton.dataset.calendarNav),
      1,
    );
    calendarDay = dateKey(calendarMonth);
    render();
    return;
  }
  const patient = currentPatient();
  if (event.target.id === "savePatient") {
    const name = value("patientName");
    if (!name) return alert("氏名を入力してください。");
    const id =
      value("patientIdentifier") || `PAT-${Date.now().toString().slice(-6)}`;
    ...
    patients.push({
      ...
      birth: value("patientBirth"),
      history: value("patientHistory"),
      chiefComplaint: value("patientChiefComplaint"),
      department: value("patientDepartment") || "未設定",
      status: value("patientStatus"),
      summary: value("patientSummary"),
      vitals: [],
      vitalSeries: [],
      calendarEvents: [],
      records: [],
  ...
  if (!patient) return;
  if (event.target.id === "saveSummary") {
    patient.summary = value("summaryText");
    patient.status = value("patientStatusEdit");
    patient.department = value("patientDepartmentEdit");
    patient.chiefComplaint = value("patientChiefComplaintEdit");
    patient.history = value("patientHistoryEdit");
    saveToICloud();
    render();
    return;
  }
  if (event.target.id === "deletePatient") {
    if (!confirm(`「${patient.label}」の患者情報と記録を削除します。`)) return;
    patients = patients.filter((item) => item.key !== patient.key);
    if (clinicalDirectoryHandle) {
      try {
        await clinicalDirectoryHandle.removeEntry(patientFileName(patient));
      } catch (error) {}
    }
    selectedId = patients[0]?.key || "";
    tab = "overview";
  ...
  if (editRecordButton) {
    editingRecordIndex = Number(editRecordButton.dataset.editRecord);
    tab = "records";
    render();
    return;
  }
  if (event.target.id === "cancelRecordEdit") {
    editingRecordIndex = null;
    render();
    return;
  }
  if (event.target.id === "saveRecordEdit") {
    const record = patient.records[editingRecordIndex];
    record.title = value("recordEditTitle");
    record.date = value("recordEditDate");
    if (record.role === "nurse") {
      const vitals = {
        temp: value("vitalTemp"),
        pulse: value("vitalPulse"),
        respiratory: value("vitalRespiratory"),
      ...
      record.body = `S：${value("subjective")}\nO：${value("objective")}`;
      record.details = `体温 ${vitals.temp || "—"}℃ ／ 脈拍 ${vitals.pulse || "—"} ／ 呼吸 ${vitals.respiratory || "—"} ／ SpO₂ ${vitals.spo2 || "—"}% ／ BP ${vitals.sys || "—"}/${vitals.dia || "—"} ／ 身長 ${vitals.height || "—"}cm ／ 体重 ${vitals.weight || "—"}kg ／ 摂取/排出 ${vitals.intake || "—"}/${vitals.output || "—"}mL ／ 疼痛 ${vitals.pain || "—"}`;
      const index = patient.vitalSeries.findIndex(
        (item) => item.recordId === record.id,
      );
      const next = { ...vitals, date: record.date, recordId: record.id };
      if (index >= 0) patient.vitalSeries[index] = next;
      else patient.vitalSeries.push(next);
    } else {
      record.body = value("recordEditBody");
      record.details = value("recordEditDetails");
    }
    record.time = "編集済み";
    editingRecordIndex = null;
    saveToICloud();
  ...
  if (event.target.id === "saveOrders") {
    patient.orders = value("ordersText");
    saveToICloud();
    render();
  }
  if (event.target.id === "saveSchedule") {
    patient.schedule = value("scheduleText");
    saveToICloud();
    render();
  }
});
