...
function chartsPanel(patient){
  ...
  const graph=(label,key,unit)=>{
    const points=series.map((item,index)=>({value:Number(item[key]),date:item.date||`記録${index+1}`,time:item.time||""})).filter(item=>Number.isFinite(item.value)&&item.value>0).sort((a,b)=>`${a.date}T${a.time}`.localeCompare(`${b.date}T${b.time}`));
