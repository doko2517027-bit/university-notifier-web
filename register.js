...
function updateState() {
  ...
  const selected = selectedDepartment || selectedMajor;
...
button.addEventListener("click", async () => {
  ...
  if (departmentCode === "10" && (number < 1 || number > 200)) {
    alert("学生番号が正しくありません。");
    return;
  }
  ...
  try {
    ...
    if (!verificationResponse.ok || !verification.verified) {
      alert("学生ページIDまたはパスワードを確認できませんでした。");
    ...
    alert("学生ページの認証に失敗しました。時間をおいて再度お試しください。");
  ...
  try {
    const userRef = doc(db, "users", studentNumber.value);
    ...
    if (userSnap.exists()) {
      await updateDoc(userRef, {
        ...
        manabaSetupSkipped: manabaPassword.value.trim() === "",

        activeMailSetupSkipped: activeMailPassword.value.trim() === "",
