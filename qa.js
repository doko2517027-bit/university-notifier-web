...
import { loadTestProgress, saveTestProgress, scrubberHtml, setupScrubber } from "./test_session.js";
...
async function loadQuestions() {
    ...
    const [questionSnap, subjectSnap] = await Promise.all([
        getDoc(doc(db, "examSubjects", subjectId, "units", unitId, "publishedQuestions", "published")),
        getDoc(doc(db, "examSubjects", subjectId))
    ]);
    subjectName = subjectSnap.data()?.name || subjectId;
    questions = (questionSnap.data()?.qa || []).filter(item =>
    ...
    currentIndex = await loadTestProgress("qa", subjectId, unitId, questions.length);
    renderQuestion();
}
...
function renderQuestion() {
    ...
    setupScrubber(qaArea, index => {
        currentIndex = index;
        renderQuestion();
        saveTestProgress("qa", subjectId, subjectName, unitId, currentIndex, questions.length);
    });
...
qaArea.addEventListener("click", async event => {
    ...
    renderQuestion();
    await saveTestProgress("qa", subjectId, subjectName, unitId, currentIndex, questions.length);
