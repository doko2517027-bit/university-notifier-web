...
async function loadDailyQuestion() {
  ...
  questionArea.innerHTML = `
        <div class="card setting-card test-focus-card" data-answer="${q.answer}">
            ...
            <div class="test-choice-list">${q.choices
              .map(
                (choice, index) => `
                <button
...
document.addEventListener("click", async (e) => {
  ...
  const answerButton = e.target.closest(".answer-button");
  ...
  if (isCorrect) {
    ...
    if (awarded.awarded) {
      panel.insertAdjacentHTML(
        "afterbegin",
        '<div class="point-earned-effect">＋3pt</div>',
      );
      await refreshTotalPoints(questionArea);
    }
  } else {
...
window.addEventListener("beforeunload", (event) => {
  ...
  const questionCard = document.querySelector(".test-focus-card");

  if (!questionCard) {
    return;
  }

  const unfinished = questionCard.dataset.finished !== "true";
