...
async function showAnnualTransitionIfRequired(user, system = {}) {
  ...
  annualTransitionTitle.textContent = finalYear
    ? "卒業・進路予定の確認"
    : "次年度の予定を確認";
  annualTransitionMessage.textContent =
    "該当する項目を選んで確定してください。";
  annualTransitionActions.innerHTML = choices
    .map(
      ([action, label]) => `
...
annualTransitionActions?.addEventListener("click", async (event) => {
  ...
  if (!confirm(`「${labels[action]}」として回答します。よろしいですか？`))
    return;
...
async function showCreditConfirmationIfRequired(user, system = {}) {
  ...
  const enrolledSnap = await getDocs(
    collection(db, "users", studentNumber, "enrolledSubjects"),
  );
...
saveCreditConfirmation?.addEventListener("click", async () => {
  ...
  try {
    ...
    items.forEach((item) => {
      ...
      const status =
        item.querySelector("select")?.value === "not_earned"
          ? "not_earned"
          : "earned";
