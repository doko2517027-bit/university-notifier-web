...
export async function awardDailyQuestionPoints({
  type,
  points,
  subjectId,
  subjectName,
  ...
  const awardId = await stableId(
    `${day}|${type}|${subjectId}|${unitId}|${questionId}`,
  );
  ...
  const result = await runTransaction(db, async (transaction) => {
    if ((await transaction.get(awardRef)).exists())
    ...
    transaction.set(awardRef, {
      ...
      questionId,
      points,
      correctAt: serverTimestamp(),
    });
    transaction.set(
      dailyRef,
      {
        point: incrementValue(points),
        solved: incrementValue(1),
    ...
    transaction.set(
      totalRef,
      { point: incrementValue(points), updatedAt: serverTimestamp() },
      { merge: true },
    );
  ...
  const subjectDayRef = doc(
    db,
    "users",
    studentNumber,
    "subjectPointHistory",
    `${day}_${subjectPointId}`,
  );
  const subjectResults = await Promise.allSettled([
    setDoc(subjectRef, subjectData, { merge: true }),
    setDoc(subjectDayRef, { ...subjectData, day }, { merge: true }),
  ]);
  const subjectSaved = subjectResults.every(
    (item) => item.status === "fulfilled",
  );
  if (!subjectSaved)
    console.error(
      "科目別ポイント保存エラー。Firestoreルールを確認してください。",
      subjectResults,
    );
