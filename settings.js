...
document.getElementById("sendContact").onclick=async()=>{const message=document.getElementById("contactMessage").value.trim();if(!message){alert("お問い合わせ内容を入力してください。");return}if(!confirm("この内容を管理者へ送信しますか？"))return;try{await addDoc(collection(db,"contacts"),{studentNumber,category:document.getElementById("contactCategory").value,message,status:"new",createdAt:serverTimestamp()});document.getElementById("contactMessage").value="";alert("送信しました。返信は『お問い合わせの履歴・返信を見る』から確認できます。")}catch(error){console.error("お問い合わせ送信エラー:",error);alert("送信できませんでした。ログインし直してからもう一度お試しください。")}};
